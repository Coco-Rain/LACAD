...
class CompressionNetVAE(nn.Module):
    def __init__(self, in_dim=768, latent_dim=64):
        ...
        if in_dim == 768:
            self.fc1 = nn.Linear(in_dim, in_dim // 2)       # 768 → 384
            self.norm1 = nn.LayerNorm(in_dim // 2)
            self.fc2 = nn.Linear(in_dim // 2, in_dim // 4)   # 384 → 192
            self.norm2 = nn.LayerNorm(in_dim // 4)
            self.final_dim = in_dim // 4                     # 192
        elif in_dim == 512:
            self.fc1 = nn.Linear(in_dim, 256)                # 512 → 256
            self.norm1 = nn.LayerNorm(256)
            self.fc2 = nn.Linear(256, 128)                   # 256 → 128
            self.norm2 = nn.LayerNorm(128)
            self.final_dim = 128
        else:
            raise ValueError("Unsupported in_dim, must be 512 or 768")
