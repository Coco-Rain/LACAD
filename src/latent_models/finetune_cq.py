...
sys.path.append(r"D:\Develop\refence_model\StructCoder-master")
...
def main():
    ...
    if args.resume == 1:
        pretrain_model = 'saved_models/pretrain/checkpoint_best.bin'
        args.logger.write('Resuming training from ckpt at ' + pretrain_model)
    ...
    for step in train_bar:
        ...
        code_autoencoder = model(inputs=batch_io['code_inputs'], outputs=batch_io['code_outputs'])
        #text_autoencoder = model(inputs=batch_io['text_inputs'], outputs=batch_io['text_outputs'])
        # ret_text2code = model(inputs=batch_io['text_inputs'], outputs=batch_io['code_outputs']) # text2code

        # compute loss
        #loss = code_autoencoder['lm_loss'] + text_autoencoder['lm_loss']
        loss = code_autoencoder['lm_loss']
        # backward pass
        loss.backward()
        ...
        if (step + 1) % args.validate_every == 0:
            ...
            results = code_ae_test(model, valid_io, valid_langs, args, train_step=step)
            model.train()

            # Save model and validation predictions if there is an improvement.
            #curr_val_metric = sum(results['CodeBLEU'].values())
            curr_val_metric = results['CodeBLEU']['autoencoder']['python'][0]
