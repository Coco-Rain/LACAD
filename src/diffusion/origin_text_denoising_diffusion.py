...
class GaussianDiffusion(nn.Module):
    ...
    def diffusion_model_predictions(self, z_t, mask, t, *, x_self_cond = None,  class_id=None, seq2seq_cond=None, seq2seq_mask=None, sampling=False, cls_free_guidance=1.0, l2_normalize=False):
        ...
        model_output = self.diffusion_model(z_t, mask, t, x_self_cond, class_id=class_id, seq2seq_cond=seq2seq_cond, seq2seq_mask=seq2seq_mask)
        if cls_free_guidance!=1.0:
            if exists(class_id):
                unc_class_id = torch.full_like(class_id, fill_value=self.diffusion_model.num_classes)
            else:
                unc_class_id = None
            unc_model_output = self.diffusion_model(z_t, mask, t, x_self_cond, class_id=unc_class_id, seq2seq_cond=None, seq2seq_mask=None)
            model_output = model_output*cls_free_guidance + unc_model_output*(1-cls_free_guidance)

        pred_v = model_output
        time_as_shape = right_pad_dims_to(z_t, t)
        x_start = z_t + (1 - time_as_shape) * pred_v
        pred_noise = x_start - pred_v

        if l2_normalize:
            assert sampling, "L2 normalization should only be used during sampling."
    ...
    @torch.no_grad()
    def ode_sample(self, shape, lengths, class_id, seq2seq_cond, seq2seq_mask, cls_free_guidance=1.0, l2_normalize=False,
                  invert=False, z_t=None, num_steps=100):

        batch, device = shape[0], next(self.diffusion_model.parameters()).device
        self.sampling_timesteps = num_steps

        times = torch.linspace(0., 1., self.sampling_timesteps + 1, device=device)
        if invert:
        ...
        if invert:
            assert exists(z_t), "z_t (the data latent) must be provided for inversion."
            z = z_t
        else:
        ...
        for time, time_next in tqdm(time_pairs, desc='Rectified Flow sampling', total=self.sampling_timesteps):
            # --- FIX: Expand scalar time to a batch-sized tensor ---
            # The model expects a time tensor of shape [batch], but the loop provides a scalar.
            # We create a tensor of the correct shape and fill it with the current time value.
            time_batch = torch.full((batch,), time.item(), device=device)

            # Call the prediction function with the corrected time_batch
            model_output = self.diffusion_model_predictions(
                z,
                mask,
                time_batch,  # Use the batch-sized time tensor here
                class_id=class_id,
                x_self_cond=x_start,
            ...
            pred_v = model_output.pred_v

            if self.self_condition:
                x_start = model_output.pred_x_start.detach()

            step_size = time_next - time
            z = z + step_size * pred_v
    ...
    def sample(self, batch_size, length, class_id=None, seq2seq_cond=None, seq2seq_mask=None, cls_free_guidance=1.0, l2_normalize=False):
        ...
        elif self.sampler == 'ode':
            sample_fn = self.ode_sample
        else:
    ...
    def forward(self, txt_latent, mask, class_id, seq2seq_cond=None, seq2seq_mask=None, return_x_start=False, *args, **kwargs):
        ...
        time_as_shape = right_pad_dims_to(txt_latent, times)
        z_t = time_as_shape * txt_latent + (1 - time_as_shape) * noise

        target = txt_latent - noise
        ...
        pred = self.diffusion_model(z_t, mask, times, x_self_cond=self_cond, class_id=class_id, seq2seq_cond=seq2seq_cond, seq2seq_mask=seq2seq_mask)
        loss = self.loss_fn(pred, target, reduction='none')

        loss = rearrange([reduce(loss[i][:torch.sum(mask[i])], 'l d -> 1', 'mean') for i in range(txt_latent.shape[0])], 'b 1 -> b 1')

        if return_x_start:
            # We can still return the predicted x_start for logging or other purposes.
            # We derive it again here using the final prediction.
            final_pred_x_start = z_t + (1 - time_as_shape) * pred
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.dataloader = text_dataset.get_dataloader(args, 'train', self.dataset['train'], self.tokenizer, self.max_seq_len,
                                                      context_tokenizer=self.context_tokenizer)
        self.val_dataloader = text_dataset.get_dataloader(args, 'val', self.dataset['valid'], self.tokenizer, self.max_seq_len,
                                                          shuffle=False, context_tokenizer=self.context_tokenizer)
        self.test_dataloader = text_dataset.get_dataloader(args, 'test', self.dataset['test'], self.tokenizer, self.max_seq_len,
                                                           shuffle=False, context_tokenizer=self.context_tokenizer)
        if not self.seq2seq:
