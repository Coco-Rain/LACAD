...
from transformers import get_scheduler, AutoTokenizer, PreTrainedTokenizerBase, T5ForConditionalGeneration, MT5ForConditionalGeneration, T5EncoderModel
...
from latent_models.finetune_cq_utils import * #load_model, parse_args,
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        if self.accelerator.is_main_process:
            ...
            run = 'diff_codet5_latent_32_128_demo1'
            if args.wandb_name:
        ...
        if 'bart' in args.enc_dec_model:
            self.context_model = BartForConditionalGeneration.from_pretrained(args.enc_dec_model)
        elif 'flan-t5' in args.enc_dec_model:
            self.context_model = T5ForConditionalGeneration.from_pretrained(args.enc_dec_model, torch_dtype=torch.bfloat16)
        elif 'codet5' in args.enc_dec_model:
            #self.context_model = T5ForConditionalGeneration.from_pretrained(args.enc_dec_model, torch_dtype=torch.bfloat16)
            self.context_model = T5EncoderModel.from_pretrained(args.enc_dec_model, torch_dtype=torch.bfloat16)
        elif 'mt5' in args.enc_dec_model:
            self.context_model = MT5ForConditionalGeneration.from_pretrained(args.enc_dec_model, torch_dtype=torch.bfloat16)
        else:
            raise ValueError(f'invalid enc_dec_model {args.enc_dec_model}')
        self.context_tokenizer = AutoTokenizer.from_pretrained(args.enc_dec_model)
        ...
        if args.latent_model_path:
            latent_model_args = parse_args()

            self.ae_model = load_model(latent_model_args)
            self.tokenizer = self.context_tokenizer

            self.diffusion.context_encoder = self.context_model
            ...
            self.diffusion.max_seq_len = latent_model_args.mem_slots_len
            self.num_encoder_latents = latent_model_args.mem_slots_len
            ...
            self.diffusion.l2_normalize = False
        ...
        dataset = text_dataset.get_code_dataset(dataset_name,)
        ...
        #self.train_val_dataloader = text_dataset.get_cq_dataloader(args, dataset['train'].select(range(100)), self.tokenizer, self.max_seq_len, shuffle=False, context_tokenizer=self.context_tokenizer)
        if args.resume_training:
            dataset['train'] = dataset['train'].shuffle()
        self.dataloader = text_dataset.get_dataloader(args, self.dataset['train'], self.tokenizer, self.max_seq_len, context_tokenizer=self.context_tokenizer)
        self.val_dataloader = text_dataset.get_dataloader(args, self.dataset['valid'], self.tokenizer, self.max_seq_len, shuffle=False, context_tokenizer=self.context_tokenizer)
        self.test_dataloader = text_dataset.get_dataloader(args, self.dataset['test'], self.tokenizer, self.max_seq_len, shuffle=False, context_tokenizer=self.context_tokenizer)
        ...
        self.diffusion, self.ae_model, self.opt, self.dataloader, self.lr_scheduler = self.accelerator.prepare(self.diffusion, self.ae_model, self.opt, self.dataloader, lr_scheduler)
    ...
    @torch.no_grad()
    def generate_test(self, num_samples=None, split='val', seed=42, num_candidates=None, cls_free_guidance=1.0, incremental=False):
        ...
        for batch in dataloader:
            ...
            latents = self.ae_model.module.get_latent_outputs(input_ids=data['input_ids'], attention_mask=data['attention_mask'])
            if self.latent_model_path:
                attention_mask = None
                encoder_output = BaseModelOutput(
                    last_hidden_state=self.ae_model.module.get_decoder_input(latents.clone()))

            sample_ids = self.ae_model.module.generate(encoder_outputs=encoder_output, attention_mask=attention_mask,
                                                       **gen_kwargs)
            ...
            pred_texts.append(texts_list)




    @torch.no_grad()
    def sample_text2code(self, num_samples=None, split='val', seed=42, num_candidates=None, cls_free_guidance=1.0, incremental=False):
        assert split in ['train', 'val', 'test']
        num_samples = default(num_samples, self.num_samples) if split != 'test' else len(self.dataset['test'])
        num_candidates = default(num_candidates, self.seq2seq_candidates)
        accelerator = self.accelerator
        ...
        for batch in dataloader:
            data = batch.to(device)
            seq2seq_cond = diffusion.context_encoder(input_ids=data['cond_input_ids'], attention_mask=data[
                'cond_attention_mask']).last_hidden_state.float()
            ...
            for _ in range(num_candidates):
                ...
                l2_normalize = False
                latents, mask = self.ema.ema_model.sample(batch_size=seq2seq_cond.shape[0], length=None,
                ...
                sample_ids = self.ae_model.module.generate(encoder_outputs=encoder_output, attention_mask=attention_mask, **gen_kwargs)
                ...
                ref_cand_list.append(
                    [self.tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=True).strip() for g
                     in data['input_ids']])
                source_cand_list.append([self.context_tokenizer.decode(g, skip_special_tokens=True,
                                                                       clean_up_tokenization_spaces=True).strip() for g
                                         in data['cond_input_ids']])
        ...
        if 'code' or 'cq_code' in self.dataset_name:
            tokenize = 'intl' if self.dataset_name == 'wmt14-en-de' else '13a'

            if num_candidates > 1:
                mbr_codebleu_scores = np.zeros((num_samples, num_candidates))
                for i in range(num_candidates):
                    ...
                    for j in range(num_candidates):
                        ...
                        # 计算CodeBLEU分数，lang="python"指定了语言类型，weights可以根据需求调整
                        codebleu_arr = np.array(
                            [calc_codebleu([ref], [pred], lang="python", weights=(0.25, 0.25, 0.25, 0.25))['codebleu']
                             for pred, ref
                             in zip(pred_texts_i, ref_texts_j)])
                        mbr_codebleu_scores[:, i] += codebleu_arr

                # 找出最佳的候选翻译
                best_indices = np.argmax(mbr_codebleu_scores, axis=1)
                ...
                # 计算最终的CodeBLEU分数
                metrics[f'model/seq2seq/{prefix}mbr_codebleu'] = calc_codebleu(best_predictions, gt_reference_texts,
                                                                               lang="python",
        ...
        if 'code' or 'cq' in self.dataset_name:
            ...
            # 计算 CodeBLEU 指标，替代 CodeBERTScore
            codebleu_scores = calc_codebleu(pred_texts, reference_texts, lang="python", weights=(0.25, 0.25, 0.25, 0.25))
            codebleu_score = codebleu_scores['codebleu']

            # 将 CodeBLEU 结果存入 metrics
            metrics[f"model/seq2seq/{prefix}codebleuscore"] = codebleu_score

            if metrics[f'model/seq2seq/{prefix}codebleuscore'] > self.best_seq2seq_metric and split == 'val' and cls_free_guidance == 1.0:
                self.best_seq2seq_metric = metrics[f'model/seq2seq/{prefix}codebleuscore']
                self.save(best=True)
    ...
    def sample_seq2seq(self, num_samples=None, split='val', seed=42, num_candidates=None, cls_free_guidance=1.0, incremental=False):
        ...
        # if 'wmt' or 'cq_code' in self.dataset_name:
        #     tokenize = 'intl' if self.dataset_name == 'wmt14-en-de' else '13a'
        #
        #     if num_candidates > 1:
            ...
            if num_candidates > 1:
                ...
                for i in range(num_candidates):
                    ...
                    for j in range(num_candidates):
                        ...
                        # 计算CodeBLEU分数，lang="python"指定了语言类型，weights可以根据需求调整
                        codebleu_arr = np.array(
                            [calc_codebleu([ref], [pred], lang="python", weights=(0.25, 0.25, 0.25, 0.25))['codebleu']  for pred, ref
                             in zip(pred_texts_i, ref_texts_j)])
                        mbr_codebleu_scores[:, i] += codebleu_arr
        ...
        if num_candidates > 1:
            mbr_codebert_scores = np.zeros((num_samples, num_candidates))
            for i in range(num_candidates):
                pred_texts_i = pred_texts[i::num_candidates]
                for j in range(num_candidates):
                    if j == i:
                        continue
                    ref_texts_j = pred_texts[j::num_candidates]
                    # 使用 CodeBERTScore 计算相似度分数
                    bert_scores = code_bert_score.score(cands=pred_texts_i, refs=ref_texts_j, lang='python')
                    mbr_codebert_scores[:, i] += bert_scores[2].numpy()  # bert_scores[2] 是 F1 分数

            # 找出最佳候选翻译
            best_indices = np.argmax(mbr_codebert_scores, axis=1)
            best_predictions = [pred_texts[i * num_candidates + idx] for i, idx in enumerate(best_indices)]
            ...
            # 最终计算 CodeBERTScore
            final_scores = code_bert_score.score(cands=best_predictions, refs=gt_reference_texts, lang='python')
            metrics[f'model/seq2seq/{prefix}mbr_codebertscore'] = final_scores[0].mean().item()  # F1 平均分
        ...
        if 'wmt' or 'cq' in self.dataset_name:
            save_path = os.path.join(self.results_folder, f'{prefix}{split}_samples.txt')   
            # Create dir if it doesn't exist
            if not os.path.exists(os.path.dirname(save_path)):
                os.makedirs(os.path.dirname(save_path))
            ...
            assert len(pred_texts) == len(reference_texts)
            #codebleu_scores = calc_codebleu(pred_texts, reference_texts,lang="python",weights=(0.25, 0.25, 0.25, 0.25))
            #codebleu_score = codebleu_scores['codebleu'][0].mean().item()
            #metrics[f"model/seq2seq/{prefix}codebleuscore"] = codebleu_score
            # 计算 CodeBERTScore，使用 CodeBERTScore 代替原来的 compute_sacrebleu
            bert_scores = code_bert_score.score(cands=pred_texts, refs=reference_texts, lang='python')
            codebert_score = bert_scores[2].mean().item()  # 使用 F1 分数的平均值
            # 将计算结果存入 metrics
            metrics[f"model/seq2seq/{prefix}codebertscore"] = codebert_score
            if metrics[f'model/seq2seq/{prefix}codebertscore'] > self.best_seq2seq_metric and split == 'val' and cls_free_guidance == 1.0:
                self.best_seq2seq_metric = metrics[f'model/seq2seq/{prefix}codebertscore']
                self.save(best=True)
            """
            sacrebleu_score = evaluation.compute_sacrebleu(pred_texts, reference_texts, tokenize=tokenize)
            metrics[f"model/seq2seq/{prefix}sacrebleu"] = sacrebleu_score
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for grad_accum_step in range(self.gradient_accumulate_every):
                    ...
                    with torch.no_grad():
                        latent = self.ae_model.module.get_latent_outputs(input_ids=data['input_ids'], attention_mask=data['attention_mask'])
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step % self.save_and_sample_every == 0:
                        #self.generate_test(cls_free_guidance=1.0, incremental=False)
                        if self.seq2seq:
                            if 'code' or 'cq' in self.args.dataset_name:
                                for guidance_strength in [1.0, 2.0]:
                                    self.sample_text2code(cls_free_guidance=guidance_strength, incremental=False)
                            else:
                                self.sample_seq2seq()
